...
for t = ts
    ...
    e_current = Q(:,1:2) - Qdes(:,1:2);
    
    %Calculate current reference velocity, reference acceleration
    qr_d_current = Qr_d(Qdes(:,2), e_current(:,1));
    qr_dd_current = Qr_dd(Qdes(:,3), e_current(:,2));
    ...
    Q_dd = M(q1,q2) \ (F(tau1,tau2,[q1_d,q2_d]) - C(q1,q2,q1_d,q2_d)*Q(:,2) - K()*Q(:,1) - G());
